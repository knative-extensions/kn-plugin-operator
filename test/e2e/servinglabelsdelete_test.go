//go:build servinglabeldelete
// +build servinglabeldelete
...
// TestServingLabelDelete verifies whether the operator plugin can configure the labels, annotations and node selectors for Knative Serving
func TestServingLabelDelete(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		name: "Knative Serving verifying the deletion of the first key-value pair for nodeSelectors",
		expectedLabels: common.KeyValueFlags{
			Value:        resources.TestValue,
			Key:          resources.TestKey,
			Component:    "serving",
		...
		name: "Knative Serving verifying the deletion of the additional key-value pair for nodeSelectors",
		expectedLabels: common.KeyValueFlags{
			Value:        resources.TestValueAdditional,
			Key:          resources.TestKeyAdditional,
			Component:    "serving",
		...
		t.Run(tt.name, func(t *testing.T) {
			resources.VerifyKnativeServingLabelsDelete(t, clients.Operator.KnativeServings(resources.ServingOperatorNamespace),
				tt.expectedLabels)
		})
	}
}
